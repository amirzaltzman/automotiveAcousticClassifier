...
from draw_confussion_matrix import *
import argparse
import collections
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('dropout'):
        ...
        dropout_bottleneck = tf.nn.dropout(bottleneck_input, 0.5)
    with tf.name_scope('biases'):
      layer_biases = tf.Variable(tf.zeros([class_count]), name='final_biases')
      variable_summaries(layer_biases)
    with tf.name_scope('Wx_plus_b'):
      logits = tf.matmul(dropout_bottleneck, layer_weights) + layer_biases
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  '''
  decoded =[]
  for one_hot_tensor in test_ground_truth:
      decoded.append(np.argmax(one_hot_tensor))
  '''

  plot_matrices(test_ground_truth, predictions)
